...
class ADS1x15:
  ...
  def _conversion_value(self, raw_adc):
    """ Returns a converted signed integer value.
    """
    raise NotImplementedError("Must be implemented by subclass.")
  ...
  def _read(self, pin):
    ...
    config = _ADS1X15_CONFIG_OS_SINGLE self.mode == Mode.SINGLE else 0
  ...
  def _read_register(self, reg, fast=False):
    ...
    with self.i2c_device as i2c:
      if fast:
        i2c.readinto(self._i2c_addr, _buf, end=2)
      else:
        i2c.write_then_readinto(self._i2c_addr, _reg, _buf, in_end=2)
    '''
    with self.i2c_device as i2c:
      if fast:
        i2c.readinto(_buf, end=2)
      else:
        i2c.write_then_readinto(bytearray([reg]), _buf, in_end=2)
    '''
